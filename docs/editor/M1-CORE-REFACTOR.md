...
状态：数据/预览边界已拆分，普通参数驱动的 Mesh 位置 Keyform 已贯通；完整 M1 尚未验收。
...
- `DrawableFrame` 使用运行坐标，包含画布、源 revision、实际参数值、几何/UV/索引和绘制字段。颜色、混合和遮罩目前仍为默认值，并不代表这些属性的编辑与导出已经实现。
...
Parameter 保存内部 ID、运行 ID、名称、范围、默认值和 decimal_places。当前支持 1–3 个普通非循环参数轴驱动 Mesh 位置。

MeshBinding 显式保存轴顺序；每个 MeshKeyform 保存该轴顺序下的关键值组合及完整位置数组。提交时检查所有组合恰好出现一次，并规范化为轴 0 最快变化的顺序。重复轴、非递增关键值、缺形态、重复组合、未知参数、长度错误与非有限值均拒绝。一个 Mesh 的位置不能被两个 Binding 同时驱动。
...
`references_to` 与 `erase_object` 要求先显式解绑，失败返回引用者。绑定可原子替换目标或参数轴。带 Keyform 的拓扑变更必须调用 `replace_mesh_with_keyforms`，提交所有新形态和每个新顶点对应的旧顶点 ID（新增顶点用空映射）；缺数据或引用错误时保持原 Document 不变。

## 算法复用与原型隔离

从 Purism 原有 `param.c` / `interpolate.c` 提取了普通数据输入的公共头 `include/PurismKeyform.h`：
...
旧 Rotation/Warp 数据与算法移至 `legacy_deformer.hpp` / `legacy_deformers.cpp`。旧算法入口显式命名为 `evaluate_legacy_mesh`，仅供原型行为访问与回归；正式求值、Godot 正常预览和编码器均拒绝含旧 deformer 的模型。新的 Rotation/Warp 数据和运行等价算法仍是下一阶段，不能将现有 legacy 类型视为已完成。

超出绑定关键值范围的对象按 Core 语义不可见，透明度保持其独立属性。不可见对象的 Core 顶点缓冲可能保留之前帧的数据；无状态求值输出零位置，消费者必须先判断 visible，不能绘制或比较未启用对象的历史缓冲。
...
- 原型快照版本升为 4，完整保存目前的参数/Binding/位置 Keyform，不保存临时预览值。没有提供旧版本迁移，版本 1–3 明确拒绝。Document 数据接口 schema_version 为 2。

现有 MeshView 只是 M1 的临时绘制适配器，尚未变成 M4 公共 renderer；当前每次刷新重建预览几何，不宣称已完成性能优化或 GPU 画面对照。
...
# 旧 validate_m1_static.py 保留为同一入口的兼容转发。
...
- Godot 报告：`target/kasane/godot-boundary/report.json`。42 项 headless 集成检查覆盖脱离场景树编辑、多个预览、资源错误、预览生命周期、源数据保存重开、失败原子性与句柄失效。它不替代 GPU 验收。
...
预先写入 `.godot/extension_list.cfg`，让扩展在启动阶段加载，首次 editor import 返回 0；再次打开已经扫描过的项目也返回 0。headless 边界脚本使用前一种方式，42 项结果只覆盖数据与预览集成。复现日志分别位于 `target/kasane/godot-baseline-import/import.log` 和 `target/kasane/godot-boundary/import.log`；修复首次动态导入仍需单独跟进。

剩余 M1：正式 Rotation/Warp 与嵌套求值、Part、可编辑绘制属性/遮罩、这些对象的完整编辑回归、通用资源包发布、GPU 图像证据。M1 整体仍为未验收。
