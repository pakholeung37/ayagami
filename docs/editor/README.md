...
Stage 00、Stage 01 已在 macOS arm64 完成并验收，其余核心阶段尚未开始。已有 runtime 能力不等同于已完成后续 Editor 阶段。

Stage 00 已交付可复用的内存网格创建、更新与释放接口，Stage 01 的 Document 已复用该接口。接下来只细化阶段 02，避免一次性把所有阶段写成代码级计划。
