...
| [原生预览 demo](../../demos/kasane-preview) | GDScript 执行、可选 UndoRedo Action、逻辑和 GPU 回归 | 正式应用使用正式模块；demo 不承担产品入口 |
...
旧 P1–P4 编排废止。以下 M1–M6 均为**待实施、未验收**；M1 是下一交付目标。

| 里程碑 | 交付结果 | 依赖 |
|---|---|---|
| [M1 Document 与 MOC3 导出](milestones/M1-document-moc3.md) | 代码创建可编辑模型，内存求值，导出文件并在运行时驱动 | 现有 core / 运行时 |
