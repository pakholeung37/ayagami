...
This checkout has been adapted for the complete Cubism SDK for Native 5-r.5
(Core and Framework) while retaining the repository's pinned Godot 4.3
`godot-cpp` baseline. New 5-r.5 blend modes fall back to the closest renderer
already provided by GDCubism; Normal, Add/Add Compatible, and
Multiply/Multiply Compatible retain their existing behavior.
