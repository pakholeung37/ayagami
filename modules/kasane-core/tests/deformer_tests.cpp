...
const std::string DOC="11111111-1111-4111-8111-111111111111", ASSET="22222222-2222-4222-8222-222222222222", MESH="33333333-3333-4333-8333-333333333333", ROT="44444444-4444-4444-8444-444444444444", WARP="55555555-5555-4555-8555-555555555555", OTHER="66666666-6666-4666-8666-666666666666";
bool near(Vec2 a, Vec2 b) { return std::abs(a.x-b.x)<0.0001f && std::abs(a.y-b.y)<0.0001f; }
int main() {
    Document doc; CHECK(doc.initialize(DOC,{100,100}).ok());
    CHECK(doc.add_asset({ASSET,"image","res://image",32,32}).status.ok());
    Mesh mesh{MESH,"mesh",ASSET,{0,1,2,3},{{0,0},{10,0},{10,10},{5,5}},{{0,0},{1,0},{1,1},{.5,.5}},{{{0,1,2},{0,2,3}}}};
    CHECK(doc.create_mesh(mesh).status.ok()); mesh.id=OTHER; CHECK(doc.create_mesh(mesh).status.ok());
    ...
    std::string previous;
    for (int i=0;i<17;++i) {
        Deformer d; d.id="aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa"+std::string(1,"0123456789abcdef"[i/16])+std::string(1,"0123456789abcdef"[i%16]);
        CHECK(deep.create_deformer(d).status.ok()); CHECK(deep.set_parent(d.id,previous).status.ok()); previous=d.id;
    }
    CHECK(deep.add_asset({ASSET,"image","res://image",32,32}).status.ok()); mesh.id=MESH; CHECK(deep.create_mesh(mesh).status.ok());
    CHECK(deep.set_parent(MESH,previous).status.code=="PARENT_DEPTH");
    Deformer extra; extra.id = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb";
