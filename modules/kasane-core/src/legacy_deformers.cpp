...
EditResult Document::create_deformer(Deformer d) {
    ...
    if (!valid_uuid(d.id)) return failed(Status::error("INVALID_ID", "Deformer ID must be a canonical UUID."));
    if (contains_id(d.id)) return failed(Status::error("DUPLICATE_ID", "Object ID already exists."));
    if (d.kind == DeformerKind::rotation) {
        ...
        if (!d.columns || !d.rows || d.columns > 16 || d.rows > 16 ||
            !std::isfinite(d.origin.x) || !std::isfinite(d.origin.y) ||
            !std::isfinite(d.size.x) || !std::isfinite(d.size.y) || d.size.x <= 0 || d.size.y <= 0)
            return failed(Status::error("INVALID_WARP", "Warp requires finite origin, positive size and 1–16 cells per axis."));
        ...
        if (auto s = validate_positions(d.control_points); !s.ok()) return failed(s);
    }
    auto id = d.id;
...
std::vector<std::string> Document::affected_meshes(const std::string &id) const {
    ...
    for (const auto &mesh : mesh_order_) {
        auto cursor = mesh;
        while (!cursor.empty()) {
            if (cursor == id) { affected.push_back(mesh); break; }
...
EditResult Document::set_parent(const std::string &id, const std::string &parent, bool organization) {
    ...
    for (const auto &[child, ignored] : candidate) {
        ...
        unsigned depth = 0;
        while (true) {
            if (!seen.insert(cursor).second) return failed(Status::error("PARENT_CYCLE", "Parent links must be acyclic."));
            auto it = candidate.find(cursor);
            if (it == candidate.end()) break;
            if (++depth > 16) return failed(Status::error("PARENT_DEPTH", "At most 16 parent links are supported."));
            cursor = it->second;
        }
    }
...
Status Document::evaluate_legacy_mesh(const std::string &id, std::vector<Vec2> &out) const {
    ...
    for (auto parent = parent_of(id); !parent.empty(); parent = parent_of(parent)) {
        ...
        for (auto &p : next) {
            ...
            if (!std::isfinite(px) || !std::isfinite(py) || std::abs(px) > std::numeric_limits<float>::max() || std::abs(py) > std::numeric_limits<float>::max())
                return Status::error("EVALUATION_OVERFLOW", "Deformation exceeds finite float32 coordinates.");
            p = {float(px), float(py)};
        }
    }
    out = std::move(next);
