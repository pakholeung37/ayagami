...
Status validate_render_mesh(std::span<const Vec2> positions,
    ...
    if (positions.size() < 3 || positions.size() > INT32_MAX ||
        positions.size() != uvs.size())
        return Status::error("INVALID_LENGTH", "A mesh needs matching positions/UVs and at least three vertices.");
    ...
    if (auto s = validate_positions(positions); !s.ok()) return s;
    if (auto s = validate_positions(uvs); !s.ok()) return s;
