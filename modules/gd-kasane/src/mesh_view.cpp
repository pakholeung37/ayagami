...
void KasaneMeshView::_bind_methods() {
    ClassDB::bind_method(D_METHOD("initialize", "positions", "uvs", "indices", "texture"), &KasaneMeshView::initialize);
...
Dictionary KasaneMeshView::initialize(const PackedVector2Array &positions,
                                     const PackedVector2Array &uvs,
                                     const PackedInt32Array &indices,
                                     const Ref<Texture2D> &texture) {
    if (!(OS::get_singleton()->get_thread_caller_id() == OS::get_singleton()->get_main_thread_id())) return error("WRONG_THREAD", "Rendering calls require the main thread.");
    ...
    for (int64_t i = 0; i < indices.size(); ++i) {
        if (indices[i] < 0) return error("INVALID_INDEX", "Indices cannot be negative.");
        dense.push_back(indices[i]);
    }
    if (auto status = kasane::validate_render_mesh(next, next_uvs, dense); !status.ok()) return result(status);
    Ref<ArrayMesh> next_surface;
    next_surface.instantiate();
    PackedVector3Array gpu_positions;
    gpu_positions.resize(next.size());
    for (size_t i = 0; i < next.size(); ++i) gpu_positions.set(i, {next[i].x, -next[i].y, 0});
    Array arrays;
    arrays.resize(Mesh::ARRAY_MAX);
    arrays[Mesh::ARRAY_VERTEX] = gpu_positions;
    arrays[Mesh::ARRAY_TEX_UV] = uvs;
    arrays[Mesh::ARRAY_INDEX] = indices;
    next_surface->add_surface_from_arrays(Mesh::PRIMITIVE_TRIANGLES, arrays, {}, {}, Mesh::ARRAY_FLAG_USE_DYNAMIC_UPDATE);
...
Dictionary KasaneMeshView::update_positions(const PackedVector2Array &positions) {
    if (!(OS::get_singleton()->get_thread_caller_id() == OS::get_singleton()->get_main_thread_id())) return error("WRONG_THREAD", "Rendering calls require the main thread.");
    if (surface_.is_null()) return error("NOT_INITIALIZED", "Initialize the mesh first.");
    ...
    auto next = vectors(positions);
    if (auto status = kasane::validate_positions(next); !status.ok()) return result(status);
...
void KasaneMeshView::clear() {
    ERR_FAIL_COND_MSG(!(OS::get_singleton()->get_thread_caller_id() == OS::get_singleton()->get_main_thread_id()), "Rendering calls require the main thread.");
    set_mesh(Ref<Mesh>());
