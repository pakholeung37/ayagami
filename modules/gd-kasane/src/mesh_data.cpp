...
void KasaneMeshData::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_vertex_positions", "ids", "positions"), &KasaneMeshData::set_vertex_positions);
    ClassDB::bind_method(D_METHOD("replace_geometry", "ids", "positions", "uvs", "triangles"), &KasaneMeshData::replace_geometry);
...
Dictionary KasaneMeshData::snapshot() const {
    auto *bridge = owner();
    return bridge ? bridge->get_mesh_snapshot(id_) : error("STALE_HANDLE", "The owning document was closed or replaced.");
}
String KasaneMeshData::get_name() const { return snapshot().get("name", String()); }
...
PackedVector2Array KasaneMeshData::get_positions() const { return snapshot().get("base_positions", PackedVector2Array()); }
PackedInt64Array KasaneMeshData::get_vertex_ids() const { return snapshot().get("vertex_ids", PackedInt64Array()); }
...
Dictionary KasaneMeshData::set_vertex_positions(const PackedInt64Array &vertices, const PackedVector2Array &positions) {
    auto *bridge = owner();
    return bridge ? bridge->set_vertex_positions(id_, vertices, positions) : error("STALE_HANDLE", "The owning document was closed or replaced.");
}
Dictionary KasaneMeshData::replace_geometry(const PackedInt64Array &vertices, const PackedVector2Array &positions,
                                           const PackedVector2Array &uvs, const PackedInt64Array &triangles) {
    auto *bridge = owner();
    if (!bridge) return error("STALE_HANDLE", "The owning document was closed or replaced.");
    auto data = snapshot();
    if (!bool(data["ok"])) return data;
    data["vertex_ids"] = vertices; data["base_positions"] = positions;
    data["uvs"] = uvs; data["triangles"] = triangles;
