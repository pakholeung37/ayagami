...
namespace kasane_gd {
inline std::string utf8(const godot::String &s) { return s.utf8().get_data(); }
inline godot::String string(const std::string &s) { return godot::String::utf8(s.c_str()); }
...
inline godot::PackedVector2Array vectors(std::span<const kasane::Vec2> input) {
    ...
    for (size_t i = 0; i < input.size(); ++i) out.set(i, {input[i].x, input[i].y});
    return out;
}
...
inline godot::PackedInt64Array ids(std::span<const uint32_t> input) {
    ...
    out.resize(input.size());
    for (size_t i = 0; i < input.size(); ++i) out.set(i, input[i]);
    return out;
}
}
