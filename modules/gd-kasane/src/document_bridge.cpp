...
void KasaneDocumentBridge::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("create_parameter", "description"), &KasaneDocumentBridge::create_parameter);
    ClassDB::bind_method(D_METHOD("write_binding", "description", "replace"), &KasaneDocumentBridge::write_binding, DEFVAL(false));
    ClassDB::bind_method(D_METHOD("set_mesh_keyform", "binding_id", "keys", "positions"), &KasaneDocumentBridge::set_mesh_keyform);
    ...
    ClassDB::bind_method(D_METHOD("initialize", "id", "canvas_size", "origin", "pixels_per_unit"), &KasaneDocumentBridge::initialize, DEFVAL(Vector2()), DEFVAL(1.0));
    ClassDB::bind_method(D_METHOD("add_image_asset", "id", "name", "source", "width", "height"), &KasaneDocumentBridge::add_image_asset);
...
Dictionary KasaneDocumentBridge::initialize(const String &id, Vector2 size, Vector2 origin, double pixels_per_unit) {
    ...
    return result(document_.initialize(utf8(id), {static_cast<float>(size.x), static_cast<float>(size.y), {float(origin.x),float(origin.y)}, float(pixels_per_unit)}));
}
Dictionary KasaneDocumentBridge::add_image_asset(const String &id, const String &name,
                                                const String &source, int64_t width, int64_t height) {
    if (OS::get_singleton()->get_thread_caller_id() != OS::get_singleton()->get_main_thread_id()) return error("WRONG_THREAD", "Document requires the main thread.");
    ...
    return apply(document_.add_asset({utf8(id), utf8(name), utf8(source), uint32_t(width), uint32_t(height)}));
}
...
Dictionary KasaneDocumentBridge::write_mesh(const Dictionary &d, bool replace) {
    ...
    if (d.has("runtime_id")) {
        if (d["runtime_id"].get_type() != Variant::STRING) return error("INVALID_FIELD", "runtime_id must be a string.");
        mesh.runtime_id = utf8(d["runtime_id"]);
    } else if (replace) {
        if (const auto *old = document_.get_mesh(mesh.id)) mesh.runtime_id = old->runtime_id;
    }
...
Dictionary KasaneDocumentBridge::restore_state(const Ref<KasaneDocumentState> &state) {
    ...
    auto out = result({}); out["revision"] = document_.revision();
...
Dictionary KasaneDocumentBridge::apply(const kasane::EditResult &edit) {
    ...
    for (const auto &id : edit.changes.object_ids) objects.push_back(string(id));
    for (const auto &id : edit.referrers) referrers.push_back(string(id));
    out["changed_objects"] = objects; out["referrers"] = referrers;
    if (!edit.status.ok()) return out;
    for (auto it=preview_values_.begin(); it!=preview_values_.end();) {
        if (!document_.get_parameter(it->first)) it=preview_values_.erase(it); else ++it;
    }
    if (edit.changes.kind != kasane::ChangeKind::none) emit_signal("changed", out);
    return out;
}
Dictionary KasaneDocumentBridge::get_mesh_snapshot(const String &id) const {
    ...
    out["texture_asset_id"] = string(mesh->texture_asset_id); out["runtime_id"] = string(mesh->runtime_id);
...
Dictionary KasaneDocumentBridge::get_document_summary() const {
    ...
    for (const auto &id : document_.parameter_order()) parameters.push_back(parameter_dictionary(*document_.get_parameter(id)));
    for (const auto &id : document_.binding_order()) bindings.push_back(binding_dictionary(*document_.get_binding(id)));
    out["parameters"] = parameters; out["bindings"] = bindings;
    out["canvas_origin"] = Vector2(document_.canvas().origin.x, document_.canvas().origin.y);
    out["pixels_per_unit"] = document_.canvas().pixels_per_unit;
    return out;
}
}
