...
Dictionary KasaneTextureStore::set_texture(const String &id,const Ref<Texture2D> &texture) {
    if(texture.is_null()||texture->get_width()<=0||texture->get_height()<=0) return error("INVALID_TEXTURE","Provide a loaded texture.");
    textures_[utf8(id)]=texture; emit_signal("changed"); return result({});
}
Dictionary KasaneTextureStore::load_asset(const Ref<KasaneDocumentBridge> &doc,const String &id) {
    if(doc.is_null()) return error("MISSING_DOCUMENT","Provide a Document.");
    ...
    if(!ResourceLoader::get_singleton()->exists(source,"Texture2D")) return error("MISSING_RESOURCE","Asset texture could not be loaded.");
    Ref<Texture2D> texture=ResourceLoader::get_singleton()->load(source,"Texture2D");
    if(texture.is_null()||texture->get_width()!=int64_t(asset->width)||texture->get_height()!=int64_t(asset->height))
...
Ref<Texture2D> KasaneTextureStore::get_texture(const String &id) const {
    auto it=textures_.find(utf8(id)); return it==textures_.end()?Ref<Texture2D>{}:it->second;
}
void KasaneTextureStore::clear() { textures_.clear(); emit_signal("changed"); }
}
