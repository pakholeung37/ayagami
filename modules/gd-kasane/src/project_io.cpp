...
namespace {
constexpr int64_t PROJECT_FORMAT_VERSION = 4;
...
Dictionary project_dictionary(const kasane::Document &document) {
    ...
    for (const auto &id : document.parameter_order()) parameters.push_back(parameter_dictionary(*document.get_parameter(id)));
    for (const auto &id : document.binding_order()) bindings.push_back(binding_dictionary(*document.get_binding(id)));
    doc["parameters"] = parameters; doc["bindings"] = bindings;
...
kasane::Status parse_deformers(const Dictionary &doc, kasane::Document &document) {
    ...
    for (const auto &id : document.mesh_order()) {
        ...
        if (auto status = document.evaluate_legacy_mesh(id, evaluated); !status.ok()) return status;
    }
    return {};
}
kasane::Status parse_project(const Dictionary &root, kasane::Document &document) {
    ...
    if (auto s = parse_pair(doc, "canvas_origin", source_canvas.origin); !s.ok()) return s;
    ...
    if (auto s = document.initialize(utf8(doc["id"]), source_canvas); !s.ok()) return s;
    ...
    for (int64_t i = 0; i < meshes.size(); ++i) {
        ...
        if (auto s = require(item, "runtime_id", Variant::STRING); !s.ok()) return s;
    ...
    if (auto status = parse_deformers(doc, document); !status.ok()) return status;
    for (auto field : {"parameters", "bindings"}) if (auto s=require(doc,field,Variant::ARRAY);!s.ok()) return s;
    Array parameters=doc["parameters"], bindings=doc["bindings"];
    for (int64_t i=0;i<parameters.size();++i) {
        if (parameters[i].get_type()!=Variant::DICTIONARY) return kasane::Status::error("INVALID_PROJECT","Parameter must be an object");
        kasane::Parameter p; if(auto s=parameter_from_dictionary(parameters[i],p);!s.ok()) return s;
        if(auto e=document.create_parameter(std::move(p));!e.status.ok()) return e.status;
    }
    for (int64_t i=0;i<bindings.size();++i) {
        if (bindings[i].get_type()!=Variant::DICTIONARY) return kasane::Status::error("INVALID_PROJECT","Binding must be an object");
        kasane::MeshBinding b; if(auto s=binding_from_dictionary(bindings[i],b);!s.ok()) return s;
        if(auto e=document.create_binding(std::move(b));!e.status.ok()) return e.status;
    }
    document.mark_saved();
    return {};
}
...
Dictionary KasaneProjectIO::save_project(const Ref<KasaneDocumentBridge> &owner, const String &path) {
    if (owner.is_null()) return error("MISSING_DOCUMENT", "Provide a Document.");
...
Dictionary KasaneProjectIO::open_project(const Ref<KasaneDocumentBridge> &owner, const String &path) {
    if (owner.is_null()) return error("MISSING_DOCUMENT", "Provide a Document.");
    auto &document_ = owner->document_;
    if (document_.transaction_active()) return error("TRANSACTION_ACTIVE", "Commit or cancel first.");
    if (!(OS::get_singleton()->get_thread_caller_id() == OS::get_singleton()->get_main_thread_id())) return error("WRONG_THREAD", "Document bridge requires the main thread.");
    const String absolute = ProjectSettings::get_singleton()->globalize_path(path);
    ...
    if (auto status = parse_project(json->get_data(), candidate); !status.ok()) return result(status);
