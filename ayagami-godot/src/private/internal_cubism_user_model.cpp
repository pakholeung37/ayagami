...
bool InternalCubismUserModel::model_load(
    ...
    ERR_FAIL_COND_V_MSG(!static_cast<CubismModelSettingJson *>(this->_model_setting)->GetJsonPointer(), false,
        "Could not parse Cubism model settings: " + model_pathname);
    ...
    if (strcmp(this->_model_setting->GetModelFileName(), "") == 0) {
        ...
        if (this->_model == nullptr) return false;
...
bool InternalCubismUserModel::model_load_resource()
{
    ...
    {
        // Never compact texture slots: drawable texture indices refer to this array.
        ERR_FAIL_COND_V_MSG(strcmp(this->_model_setting->GetTextureFileName(index), "") == 0,
            false, "Cubism model contains an empty texture path.");
        ...
        if (!res_loader->exists(texture_pathname)) {
            ERR_FAIL_COND_V_MSG(!FileAccess::file_exists(texture_pathname), false,
                "Cubism texture does not exist: " + texture_pathname);
            Ref<Image> img = Image::load_from_file(texture_pathname);
            ERR_FAIL_COND_V_MSG(img.is_null() || img->is_empty(), false,
                "Could not decode Cubism texture: " + texture_pathname);
            tex = ImageTexture::create_from_image(img);
            ERR_FAIL_COND_V(tex.is_null(), false);
    ...
    for (int i = 0; i < this->_model->GetDrawableCount(); ++i) {
        const int index = this->_model->GetDrawableTextureIndex(i);
        ERR_FAIL_COND_V_MSG(index < 0 || index >= this->_renderer_resource.ary_texture.size(),
            false, "Cubism drawable references an invalid texture index.");
    }
    return true;
}
